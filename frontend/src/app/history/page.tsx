...
function SimRow({ sim, compact }: { sim: JobSummary; compact?: boolean }) {
  return (
    ...
    >
      ...
      />
      {!compact && (
        <span className="shrink-0 rounded-md bg-gold/[0.08] px-2 py-0.5 text-xs font-medium text-gold">
      ...
      <div className="min-w-0 flex-1">
        {sim.player_name ? (
          <span className={`block truncate text-zinc-200 ${compact ? 'text-xs' : 'text-sm'}`}>
            {sim.player_name}
            {sim.player_class && <span className="ml-1.5 text-zinc-300">{sim.player_class}</span>}
          </span>
        ) : sim.status === 'failed' ? (
          ...
          <span className={`block truncate text-zinc-300 ${compact ? 'text-xs' : 'text-sm'}`}>
      ...
      <span
        className={`hidden shrink-0 text-right text-zinc-300 sm:block ${compact ? 'w-12 text-xs' : 'w-20 text-xs'}`}
      >
        {FIGHT_STYLE_SHORT[sim.fight_style] || sim.fight_style}
      </span>
      <span
        className={`shrink-0 text-right text-zinc-300 ${compact ? 'w-12 text-sm' : 'w-14 text-sm'}`}
      >
    ...
    <div className="border-b border-border last:border-b-0">
      ...
      <div className="flex items-center gap-3 px-5 py-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          {first?.player_name && (
            <span className="ml-2 text-xs text-zinc-300">{first.player_name}</span>
          )}
        </div>
        <span className="shrink-0 rounded-md bg-gold/[0.08] px-2 py-0.5 text-xs font-medium text-gold">
          {simType}
